  ...
  for (let i = 0; i < bookmarkList.length; i++) {
    ...
    for (let tagName in bookmarkList[i].memo) {
      bookmarkList[i].memo[tagName].forEach((memo, idx) => {
        ...
        li.id = "bk_li_container_" + bookmarkList[i].markList[tagName][idx];
        if (memo == null) {
          memo = "memo";
          li.style.color = "gray";
        }
        ...
        ael(rm_memo_btn, "click", async function () {
          ...
          setThanRemove({ list: tempBookmarkList, query: {}, url: tabUrl, data: { tagName: self.dataset.tagName, encodedContent: self.dataset.encodedContent } });
        });
        ul.append(li);
        markNum++;
      });
    ...
    titleContainer.innerHTML += `${markIcon}<h4>${title.replace(/</, "&lt;").replace(/>/, "&gt;")}</h4>${closeBtn("ct_bk_rm_btn_" + i)}`;
    ...
    titleContainer.addEventListener("click", async function (e) {
