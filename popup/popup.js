  ...
  for (let i = 0; i < bookmarkList.length; i++) {
    ...
    for (let tagName in bookmarkList[i].memo) {
      bookmarkList[i].memo[tagName].forEach((memo, idx) => {
        ...
        li.innerHTML += `<span class="ct_bk_memo">${memo.replace(/</, "&lt;").replace(/>/, "&gt;")}</span>${closeBtn(tagName + bookmarkList[i].markList[tagName][idx])}`;
        memoNum++;
        li.title = memo;
    ...
    titleContainer.innerHTML += `<div class="ct_bk_num">${markNum}</div><h3>${title.replace(/</, "&lt;").replace(/>/, "&gt;")}</h3>${closeBtn("ct_bk_rm_btn_" + i)}`;
    ...
    ael(page_rm_btn, "click", function (e) {
      let filteredBookmarkList = bookmarkList.filter((item) => item.url !== this.dataset.url);
      e.stopPropagation();
      document.getElementById(ulContainer.id).remove();
      console.log(filteredBookmarkList);
    //   chrome.storage.local.set({ bookmarkList: filteredBookmarkList });
    });
