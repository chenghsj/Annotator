  ...
  for (let i = 0; i < bookmarkList.length; i++) {
    ...
    for (let tagName in bookmarkList[i].memo) {
      bookmarkList[i].memo[tagName].forEach((memo, idx) => {
        ...
        li.innerHTML += `<span class="ct_bk_memo">${memo.replace(/</, "&lt;").replace(/>/, "&gt;")}</span>${closeBtn("rm_memo_btn_" + tagName + idx)}`;
        memoNum++;
        li.title = memo;
        ...
        ael(rm_memo_btn, "click", async function () {
          ...
          let memoIdx = tempBookmarkList[i].markList[this.dataset.tagName].indexOf(this.dataset.encodedContent);
    ...
    let markIcon = bookmarkList[i].iconUrl ? `<img class="ct_bk_icon" src=${bookmarkList[i].iconUrl}></img>` : blankIconPath;
    titleContainer.innerHTML += `<div class='bk_mm_collapse_container'><span class='bk_mm_collapse_btn'></span></div>${markIcon}<h4>${title.replace(/</, "&lt;").replace(/>/, "&gt;")}</h4>${closeBtn("ct_bk_rm_btn_" + i)}`;
  ...
  ael(ct_bk_search, "keyup", function (e) {
    ...
    searchTimeoutId = setTimeout(() => {
      ...
      document.querySelectorAll(".bk_ul_container").forEach((ul_container) => {
        if (searchInputValue !== "" && !regexp.test(ul_container.innerText.toLowerCase()) && searchInputValue !== "" && !regexp.test(ul_container.getElementsByTagName("ul")[0].innerText.toLocaleLowerCase())) {
          ul_container.style.display = "none";
        } else {
          ul_container.style.display = "block";
        }
        // ul_container.getElementsByTagName("ul")[0].innerText;
      });
    }, 200);
  });
}
