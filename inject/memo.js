...
class MemoInputBox {
   ...
   */
  constructor({ event, top, tagName, encodedContent, backgroundColor, color }) {
    this.bookmarkMessage = { tagName, encodedContent };
    this.inputBox;
    this.textarea;
    this.visible = true;
    this.top = top;
    this.create({ top, tagName, encodedContent, backgroundColor, color });
  }
  create = ({ tagName, encodedContent, backgroundColor, color }) => {
    ...
    this.inputBox.style.background = backgroundColor;
...
class MemoBox {
  constructor({ tagName, encodedContent, top, backgroundColor, color }) {
    this.memoBox;
    this.memo;
    this.createMemoBox(tagName, encodedContent, top, backgroundColor, color);
  }
  createMemoBox = (tagName, encodedContent, top, backgroundColor, color) => {
    let globalData = GlobalDataProxy.getInstance();
    let memoIdx;
    memoIdx =
      globalData.bookmarkList[globalData.bookmarkIdx].markList[tagName].indexOf(encodedContent);
    ...
    this.memoBox.style.background = backgroundColor;
    this.memoBox.style.color = color;
