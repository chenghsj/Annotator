...
async function bookmarkInit() {
  ...
  globalData.ct_text_color = items.colorList?.ct_text_color || "#000000";
  globalData.mm_bg_color = items.colorList?.mm_bg_color || "#ffff8a";
  globalData.mm_text_color = items.colorList?.mm_text_color || "#000000";
...
async function removeBookMark({ tagName, encodedContent, all, url }) {
  const resetStyle = (element) => {
    element.classList.remove("ct_bks_bg");
